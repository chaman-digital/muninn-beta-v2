...
try:
    ...
    print("Error: Paquete 'google-genai' no instalado. Ejecute: pip install google-genai")
    sys.exit(1)
...
try:
    ...
    print("Error: Paquete 'Pillow' no instalado. Ejecute: pip install Pillow")
    sys.exit(1)
...
class ConsoleLog:
    ...
    @classmethod
    def info(cls, msg):
        print(f"{cls.CYAN}[{cls.timestamp()}]{cls.END} {msg}")

    @classmethod
    def success(cls, msg):
        print(f"{cls.GREEN}[{cls.timestamp()}] ✓{cls.END} {msg}")

    @classmethod
    def warning(cls, msg):
        print(f"{cls.YELLOW}[{cls.timestamp()}] ⚠{cls.END} {msg}")

    @classmethod
    def error(cls, msg):
        print(f"{cls.RED}[{cls.timestamp()}] ✗{cls.END} {msg}")

    @classmethod
    def header(cls, msg):
        width = 64
        border = "═" * width
        print(f"\n{cls.BOLD}{cls.BLUE}╔{border}╗{cls.END}")
        print(f"{cls.BOLD}{cls.BLUE}║{cls.END}  {msg:<{width - 2}}{cls.BOLD}{cls.BLUE}║{cls.END}")
...
def _scan_single_dir(directory: str, recursive: bool, seen_paths: set) -> list[dict]:
    ...
    if recursive:
        for root, dirs, filenames in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'.git', '.venv', '__pycache__', 'node_modules', '.DS_Store'}]
...
def filter_batch(files: list[dict], db_hashes: set, proactive_hashes: set) -> list[dict]:
    ...
    for file_info in files:
        ...
        # Si ya tiene hallazgo proactivo, saltar
        if sha in proactive_hashes:
            continue
        # Si está en la BD de files, marcar (pero sí procesar para patrones)
        if sha in db_hashes:
...
def _select_model(file_type: str) -> str:
    """Selecciona el modelo Gemini adecuado según el tipo de archivo.
    Audio y video usan Pro (razonamiento profundo para transcripción verbatim).
    Imágenes y PDFs usan Flash (OCR rápido, menor costo)."""
...
def analyze_with_gemini(filepath: str, file_type: str,
    ...
    for attempt in range(1, MAX_RETRIES_429 + 1):
        ...
        try:
            ...
            log.error(f"  JSON inválido de Gemini para {filename}: {e}")
            return None
        except Exception as e:
            error_str = str(e).lower()
            is_429 = "429" in error_str or "resource" in error_str and "exhausted" in error_str
...
def main():
    ...
    if not api_key:
        log.error("Variable de entorno GOOGLE_API_KEY no definida.")
        log.error("Configure con: export GOOGLE_API_KEY='su_clave_aqui'")
        sys.exit(1)

    client = genai.Client(api_key=api_key)
    log.success("Conexión con Gemini Pro/Flash establecida")
