...
# ── Protocolo de Ingesta Crítica V2.3.1 (Mimetype-Safe) ──
...
def _classify_by_type(all_files: list[dict]) -> list[tuple[str, int, list[dict]]]:
    """Clasifica archivos por tipo en sub-lotes (Mimetype-Safe V2.3.1):
...
def run_analysis():
    ...
    Protocolo de Ingesta Crítica V2.3.1:
      A. Texto/MD → B. PDF(<15MB) → C. Imágenes(×20) → D. Audios(×3) → E. Videos(×1)
    Dentro de cada categoría: menor a mayor peso en KB."""

    log.header("ANALISTA PROACTIVO FORENSE — MUNINN V2.3.1 (Ingesta por Prioridad y Tipo)")
    ...
    log.header("FASE 3: Procesamiento por Sub-Lotes (Protocolo V2.3.1)")
    ...
    for lote_num, (lote_name, lote_capacity, lote_files) in enumerate(sub_batches, 1):
        ...
        for idx, file_info in enumerate(pending, 1):
            ...
            global_processed += 1
            print(f" - Archivo {global_processed} de 2079 procesado", flush=True)
